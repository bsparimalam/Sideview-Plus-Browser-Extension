	...
	details => {
		for(let i = 0; i < details.requestHeaders.length; ++i) {
			if (details.requestHeaders[i].name === 'User-Agent') {
				...
				temp = temp.split('/');
				let host = temp[0];
				if ((host === 'Edg') && (isminiloading || ismousein)) {
					details.requestHeaders[i].value = edgeua;
				} else if ((host === 'OPR') && (isminiloading || ismousein)) {
					details.requestHeaders[i].value = operaua;
				} else if ((host === 'Firefox') && (isminiloading || ismousein)) {
...
function loadthepage(url) {
	...
	if (url.indexOf('.') === -1) {
		...
		setTimeout(() => {
			addressbox.placeholder = 'google.com';
			addressbox.value = '';
		}, 500);
	} else {
